...
/*Modified LevelDB client for HeteroDB work sudarsun@gatech.edu*/
...
public:
    LevelDbServer(const std::string& directoryName,
                  uint32_t writeBufferSizeMb, uint32_t blockCacheSizeMb, uint32_t num_level, uint32_t num_read_threads) : 
        directoryName_(directoryName),
        writeBufferSizeMb_(writeBufferSizeMb),
	num_levels_(num_level), 
        ...
        options.num_read_threads = num_read_threads_;
        ...
        for (directory_iterator itr(directoryName); itr != end_itr;itr++) {
            if (is_directory(itr->status())) {
                ...
                leveldb::Status status = leveldb::DB::Open(options, itr->path().string(), itr->path().string(), &db);
                assert(status.ok());
    ...
    ResponseCode::type addMap(const std::string& mapName) {
        ...
        options.num_levels = num_levels_;
        options.num_read_threads = num_read_threads_;
        leveldb::Status status = leveldb::DB::Open(options, directoryName_ + "/" + mapName, directoryName_ + "/" + mapName,  &db);
        if (!status.ok()) {
    ...
    ResponseCode::type dropMap(const std::string& mapName) {
        ...
        maps_.erase(itr);
        DestroyDB(directoryName_ + "/" + mapName, directoryName_ + "/" + mapName, leveldb::Options());
        return ResponseCode::Success;
    }
    ...
    void get(BinaryResponse& _return, const std::string& mapName, const std::string& key) {
        ...
        _return.value.assign(recordValue);
        ...
        //fprintf(stdout,"map found %s key %s value %s\n",mapName.c_str(), key.c_str(), _return.value.c_str());
	...
	//fprintf(stdout,"Before if(!blindinsert) key %s \n",key.c_str());
	...
	//fprintf(stdout,"key %s syncmode %d \n",key.c_str(),syncmode);
...
int main(int argc, char **argv) {
    ...
    config.add_options()
        ...
        ("num_levels,n", po::value<int>(&num_levels)->default_value(1), "level count")
        ("num_read_threads,t", po::value<int>(&num_read_threads)->default_value(0), "num read threads")
    ...
    shared_ptr<LevelDbServer> handler(new LevelDbServer(dir, writeBufferSizeMb, blockCacheSizeMb, num_levels, num_read_threads));
